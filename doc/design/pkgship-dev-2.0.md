...
兼容性设计：
依赖的组件需要细化到接口，依赖了redis、ES的哪些接口，哪些功能。

部署相关的步骤、流程要具体化，在易用性方面能体现；
日志内要体现操作日志流程，包含时间、用户信息、操作动作；调试日志和操作日志需要分开；（写操作，读操作）
调试日志规格：大小不大对，规格可配。平均每天记录日志规格*
操作日志规格：至少报错*天的错误
导入文件的规格需要记录清楚，需要转换到使用文档中。对于系统的最低要求，以及150m的文件属于什么样规格的文件
写入程序执行时，读接口被调用的情况需考虑

*考虑服务整体部署的时候，周边需要怎么部署，如何保证高可靠，需给用户提供建议部署方案。
如果是内存盘，1G的规格太大。磁盘即可。需写入规格要求中。

校验模块要对命令注入风险进行考虑 —— 是否有命令注入风险 —— pkgship无执行命令行的操作 —— 需要显示具体的实现方案，确认无 命令注入风险。

补充威胁分析文档，如何做层层访问的韧性，安全

兼容性：兼容、适配 接口保证兼容。适配层保证顶层接口变动最小化

包信息下载将所有信息包含在其中。

容器部署：ES部署在哪里、redis部署：依赖接口的配置。需要添加一个服务部署视图。

添加systemctl对外接口

辅助分析：周边服务的关系，系统运行情况的考虑，将考虑的逻辑流程展示清楚

配置文件只允许root用户去写，user可读不可写。需要在部署视图中呈现。
systemd启动程序时，需要建立一个服务管理员用户。
安装rpm包的时候需要建立一个普通管理员用户去管理该软件。

异常退出的界定是如何的，什么情况下会自启动
