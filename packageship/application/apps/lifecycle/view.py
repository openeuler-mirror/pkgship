...
class DownloadFile(Resource):
    ...
    def _download_excel(self, file_type, table_name=None):
        ...
        if download_content is None:
            return jsonify(ResponseCode.response_json(ResponseCode.SERVICE_ERROR))
    ...
    def __bytes_save(self, data_frame):
        ...
        try:
            ...
            data_frame.to_excel(writer, sheet_name='结果汇总', index=False)
            writer.save()
            writer.close()
            bytes_io.seek(0)
    ...
    def __get_issues_content(self):
        ...
        try:
            with DBHelper(db_name='lifecycle') as database:
                ...
                idsues_dicts = IssueDownloadSchema(many=True).dump(_issues)
                return idsues_dicts
        except (SQLAlchemyError, DisconnectionError) as error:
            current_app.logger.error(error)
            return None
    ...
    def get(self, file_type):
        ...
        if file_type not in ['packages', 'issues']:
            return jsonify(ResponseCode.response_json(ResponseCode.PARAM_ERROR))
...
class MaintainerView(Resource):
    ...
    def __query_maintainers(self, table_name):
        """
            Query the names of all maintainers in the specified table
        """
        try:
            with DBHelper(db_name='lifecycle') as database:
                model = meta_model(table_name)
                maintainers = database.session.query(
                    model.maintainer).group_by(model.maintainer).all()
    ...
    def get(self):
        ...
        maintainers = self.__query_maintainers(table_name)
        return jsonify(ResponseCode.response_json(ResponseCode.SUCCESS, maintainers))
...
class TableColView(Resource):
    ...
    def get(self):
        ...
        return jsonify(ResponseCode.response_json(ResponseCode.SUCCESS, table_mapping_columns))
